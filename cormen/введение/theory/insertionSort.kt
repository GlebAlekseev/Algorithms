...
fun insertionSort(array: Array<Int>){
    for (j in 1 until array.size){            // c1 n
        val key = array[j]                          // c2 n-1 (-1 тк 0 отбрасывается по условию)
        var i = j-1                                 // c3 n-1
        while (i > 0 && array[i] > key){            // c4 ∑n|j=2 (Tj)
            array[i+1] = array[i]                   // c5 ∑n|j=2 (Tj-1) (-1 тк i>0 отбрасывает один элемент)
            i--                                     // c6 ∑n|j=2 (Tj-1)
        }
...
// c4∑n|j=2 = (n-1)*(2+n)/2 = (n^2 + 2n - n - 2) / 2 = n(n+1)/2 -1
// c5∑n|j=2 (j-1) = (n-1)*(1 + n-1)/2 = (n-1)n/2

// Суммарное число операций
// T(n) = c1n + c2(n-1) + c3(n-1) + c4∑n|j=2 (Tj) + c5∑n|j=2 (Tj-1) + c6∑n|j=2 (Tj-1) + c7(n-1)
...
// T(n) = c1n + c2(n-1) + c3(n-1) + c4∑n|j=2 (Tj) + c7(n-1)
// T(n) = c1n + c2(n-1) + c3(n-1) + n(n+1)/2 -1 + c7(n-1)
// T(n) = Cn-C
// Линейно

// Верхняя граница
// В худшем случае условие array[i] > key всегда срабатывает, следует Tj = j
// T(n) = c1n + c2(n-1) + c3(n-1) + c4∑n|j=2 (j) + c5∑n|j=2 (j-1) + c6∑n|j=2 (j-1) + c7(n-1)
// T(n) = c1n + c2(n-1) + c3(n-1) + n(n+1)/2 -1 + (n-1)n/2 + c6∑n|j=2 (j-1) + c7(n-1)
// T(n) = Cn^2 + Cn -
// Квадратично